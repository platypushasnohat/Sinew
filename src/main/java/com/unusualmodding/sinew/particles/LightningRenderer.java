...
public class LightningRenderer {
    ...
    public class Timestamp {
        ...
        public Timestamp subtract(Timestamp other) {
            ...
            return LightningRenderer.this.new Timestamp(newTicks, newPartial);
        }
    ...
    public class BoltOwnerData {
        ...
        private Timestamp lastBoltTimestamp = LightningRenderer.this.new Timestamp();
        private Timestamp lastUpdateTimestamp = LightningRenderer.this.new Timestamp();
    ...
    public class BoltInstance {
        ...
        public void render(Matrix4f matrix, VertexConsumer buffer, Timestamp timestamp) {
            ...
            for (int i = bounds.getLeft(); i < bounds.getRight(); ++i) {
                this.renderQuads.get(i).getVecs().forEach((v) -> buffer.vertex(matrix, (float)v.x, (float)v.y, (float)v.z).color(this.bolt.getColor().x(), this.bolt.getColor().y(), this.bolt.getColor().z(), this.bolt.getColor().w()).endVertex());
            }
        }
